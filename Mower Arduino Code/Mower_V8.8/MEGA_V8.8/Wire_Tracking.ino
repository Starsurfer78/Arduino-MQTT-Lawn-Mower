...
void PrintWirePosition() {
  ...
  for (int i = PrintInMax; i <= PrintOutMax; i++) {
    if (i == PrintInMax) DPRINT(F("|"));
    if (i == PrintInMid) DPRINT(F("|"));
    if (i == PrintInMin) DPRINT(F("|"));
    if (i == PrintOutMin) DPRINT(F("|"));
    if (i == PrintOutMid) DPRINT(F("|"));
    if (i == PrintOutMax) DPRINT(F("|"));
    if (i == PrintMAG_Now) DPRINT(F("X"));     //maybe change to MAG_Lasr...
    if (i == 0) DPRINT(F("0"));

    else DPRINT(F("."));

  }
  DPRINT(F("|MAG_Now:"));  DPRINT(MAG_Now); DPRINT(F("|"));
}
...
void Track_Perimeter_Wire_To_Dock()  {
  ...
  DPRINTLN(F(" Tracking the wire to the Garage: "));                     // Prints the PID values used.
  DPRINT(F("P = "));
  DPRINT(P);
  DPRINT(F(" / D = "));
  DPRINT(D);
...
void Tracking_Restart_Blocked_Path() {
  Motor_Action_Stop_Motors();
  DPRINTLN(F("Possible Blocked Path - Trying to Avoid"));
  Mower_Running = 1;
  Tracking_Wire = 1;
