...
void Print_Membrane_Switch_Input_Settings() {
  ...
  DPRINTLN();
  DPRINTLN(F("Settings Menu Activated"));
  ...
  while (Menu_Complete_Settings == false) {                      // holds the program in a loop until a selection has been made in the membrane button menu
    ...
    if (!Start_Key_X) {
      ...
      DPRINTLN(F("Start key is pressed"));
      lcd.clear();
      Activate_Menu_Option_Settings();
    }
    if (!Plus_Key_X) {
      DPRINTLN(F("+ key is pressed"));
    ...
    if (!Stop_Key_X) {
      DPRINTLN(F("Stop key is pressed"));
      Menu_Complete_Settings = true;
      lcd.clear();
      lcd.setCursor(0, 0);
...
void Run_Menu_Order_Settings() {
  ...
  DPRINT(F("Menu View : "));
  DPRINT(Menu_View);
  DPRINT(F("| Menu Selection"));
  DPRINTLN(Menu_Mode_Selection);
  delay(100);
}
...
void Activate_Menu_Option_Settings() {
  ...
  if (Menu_Mode_Selection == 1) {
    ...
    DPRINTLN(F("Alarms Setup Selected"));
    Menu_Mode_Selection = 0;
    delay(1000);
    lcd.clear();
  ...
  if (Menu_Mode_Selection == 2) {
    ...
    DPRINTLN(F("Sensor Setup Selected"));
    Menu_Mode_Selection = 0;
    delay(1000);
    lcd.clear();
  ...
  if (Menu_Mode_Selection == 3) {
    ...
    DPRINTLN(F("Motion Setup Selected"));
    Menu_Mode_Selection = 0;
    delay(1000);
    lcd.clear();
  ...
  if (Menu_Mode_Selection == 4) {
    ...
    DPRINTLN(F("Tracking Setup Selected"));
    Menu_Mode_Selection = 0;
    delay(1000);
    lcd.clear();
  ...
  if (Menu_Mode_Selection == 10) {
    ...
    lcd.setCursor(0, 1);
    lcd.print("Yes/No");
    DPRINTLN(F("Clear EEPROM Yes/No   - Press Up for Yes and Down for No.  Start to Confirm"));
    Menu_Mode_Selection = 0;
    Menu_Complete = false;
    int Answer = 0;
    while (Menu_Complete == false) {
      ...
      if (!Plus_Key_X) {
        ...
        lcd.setCursor(0, 1);
        lcd.print("Yes    ");
        DPRINTLN(F("Clear EEPROM = YES....  Press Start to Confirm"));
        delay(100);
      }
      if (!Minus_Key_X) {
        Answer = 0;
        lcd.setCursor(0, 1);
        lcd.print("No     ");
        DPRINTLN(F("Clear EEPROM = No....Press Start to Confirm"));
        delay(100);
      }
    }
  }
