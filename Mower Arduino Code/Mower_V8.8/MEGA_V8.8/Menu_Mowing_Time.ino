...
void Print_Membrane_Switch_Input_Timing() {
  ...
  DPRINTLN();
  DPRINTLN(F("Test Menu Activated"));
...
void Activate_Menu_Option_Timing() {
  ...
  if (Menu_Mode_Selection == 1) {
    ...
    DPRINTLN(F("Maximum Mow Time Selected"));
    delay(5000);
    lcd.clear();
    Menu_Mode_Selection = 0;
    lcd.clear();
  ...
  if (Menu_Mode_Selection == 2) {
    // 1hr Mowing
    DPRINTLN(F("1 hr Mow Time Selected"));
    lcd.clear();
    Menu_Mode_Selection = 0;
    lcd.clear();
    Menu_Complete = false;
    ...
    DPRINT(F("Finish Time set to : "));
    DPRINT(Alarm_Timed_Mow_Hour);
    DPRINT(F(":"));
    if (Alarm_Timed_Mow_Minute < 10) DPRINT ("0");
    DPRINTLN(Alarm_Timed_Mow_Minute);
  ...
  if (Menu_Mode_Selection == 3) {
    // 2hr Mowing
    DPRINTLN(F("2 hr Mow Time Selected"));
    lcd.clear();
    Menu_Mode_Selection = 0;
    lcd.clear();
    Menu_Complete = false;
