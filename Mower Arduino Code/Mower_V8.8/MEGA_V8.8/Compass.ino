...
void Get_Manuel_QMC5883_Reading() {
  ...
  DPRINTLN(F("Use DF Robot and GY282"));
...
void Turn_To_Compass_Heading() {
  ...
  if (Compass_Target < 180) {
    ...
    DPRINTLN("Compass Target < 180 logic used");
    float Limit = Compass_Target + 180;
    DPRINT("Left Turn Limit : ");
    DPRINT(Compass_Target);
    DPRINT(" to ");
    DPRINTLN(Limit);

    DPRINT("Right Turn Limit : ");
    DPRINT(Limit);
    DPRINT(" to 360° & 0 to ");
    DPRINTLN(Compass_Target);
    ...
    while ((Compass_Heading_Degrees < Heading_Lower_Limit_Compass) || (Compass_Heading_Degrees > Heading_Upper_Limit_Compass) && (Attemps_Compass_Turn < 40) && (Bad_Reading < 5))  {
      ...
      if (Attemps_Compass_Turn > 5) {
        if (Compass_Heading_Degrees - Compass_Last > 50) {
          DPRINT("Bad Compass Reading ");
          DPRINT("Compass Heading Degrees = ");
          DPRINT(Compass_Heading_Degrees);
          DPRINT("  Compass Last = ");
          DPRINT(Compass_Last);
          DPRINT("  Degrees - Last = ");
