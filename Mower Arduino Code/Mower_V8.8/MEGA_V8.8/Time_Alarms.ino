...
void Print_Time_On_Serial_Monitor() {
  ...
  if (PCB == 0) {
    ...
    DPRINT(t.hr);
    DPRINT(":");
    if (t.min < 10) DPRINT ("0");
    DPRINT(t.min);
    DPRINT(".");
    if (t.sec < 10) DPRINT ("0");
    DPRINT(t.sec);
  }
  if (PCB == 1) {
    Display_DS3231_Time();
...
void Display_Next_Alarm()  {
  ...
  if (Alarm_1_ON == 1 ) {
    ...
    DPRINT(Alarm_1_Minute);
    DPRINT("|");
  }
  if (Alarm_1_ON == 0) DPRINT("|A1 OFF");
